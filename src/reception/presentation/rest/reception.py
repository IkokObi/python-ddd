...
from reception.domain.exception.reservation import (
    ReservationNotFoundException,
    ReservationStatusException,
)
from reception.domain.exception.room import RoomNotFoundException, RoomStatusException
from reception.presentation.rest.schema.request import (
    CheckInRequest,
    CreateReservationRequest,
    UpdateGuestRequest,
)
from reception.presentation.rest.schema.response import (
    ReservationResponse,
    ReservationSchema,
)
...
def post_reservations(
    ...
    reservation_command: ReservationCommandUseCase = Depends(
        Provide[AppContainer.reception.reservation_command]
    ),
):
    try:
        reservation: Reservation = reservation_command.make_reservation(
    ...
    return ReservationResponse(
        detail="ok", result=ReservationSchema.build(reservation=reservation)
    )
...
def post_reservation_check_out(
    ...
    try:
        reservation: Reservation = reservation_command.check_out(
            reservation_number=reservation_number
        )
    except ReservationNotFoundException as e:
        raise HTTPException(
...
def post_reservation_cancel(
    ...
    try:
        reservation: Reservation = reservation_command.cancel(
            reservation_number=reservation_number
        )
    except ReservationNotFoundException as e:
        raise HTTPException(

...
class ReservationCommandUseCase:
    ...
    def make_reservation(self, request: CreateReservationRequest) -> Reservation:
        with self.db_session() as session:
            room: Room | None = self.reservation_repo.get_room_by_room_number(
                session=session, room_number=request.room_number
            )
        if not room:
    ...
    def update_guest_info(
        self, reservation_number: str, request: UpdateGuestRequest
    ) -> Reservation:
        reservation: Reservation = self.reservation_query.get_reservation(

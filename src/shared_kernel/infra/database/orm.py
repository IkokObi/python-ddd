...
def init_orm_mappers():
    ...
    """
    from reception.domain.entity.reservation import (
    ...
    mapper_registry.map_imperatively(
        ...
        properties={
            "room": relationship(
                ...
                order_by=reservation_table.c.id.desc,
                lazy="joined",
            ),
            "reservation_number": composite(
                ReservationNumber.from_value, reservation_table.c.number
            ),
            "reservation_status": composite(
                ReservationStatus.from_value, reservation_table.c.status
            ),
            "guest": composite(
                Guest, reservation_table.c.guest_mobile, reservation_table.c.guest_name
            ),
        },
    )
